...
void stop_self_check(void) {
  PROCESS_ID_TYPE self;
  check_e(GET_MY_ID(&self, &ret));
  info("%lu started, stop self\n", self);
  STOP_SELF();
}

void start_check(void) {
  PROCESS_ID_TYPE tgt;
  check_e(GET_PROCESS_ID("stop_self_check", &tgt, &ret));
...
void main(void) {
  PROCESS_ATTRIBUTE_TYPE attr[2] = {{
                                        .NAME = "stop_self_check",
                                        .ENTRY_POINT = (SYSTEM_ADDRESS_TYPE)stop_self_check,
                                        .BASE_PRIORITY = 2,
                                        .STACK_SIZE = PGSIZE * 4,
                                        .DEADLINE = SOFT,
